...
if menu == "Invoice":
    ...
    # Upload files
    chargebee_file = st.file_uploader("ChargeBee Export (.xlsx)", type="xlsx", key="cb_invoice")
    quickbooks_file = st.file_uploader("QuickBooks Export (.xlsx)", type="xlsx", key="qb_invoice")
    bridge_file = st.file_uploader("Bridge (.xlsx)", type="xlsx", key="bridge_invoice")
    customers_file = st.file_uploader("Customers_MI (.xlsx)", type="xlsx", key="cust_invoice")

    if chargebee_file and quickbooks_file and bridge_file and customers_file:
        try:
            df_cb = pd.read_excel(chargebee_file)
            df_qb = pd.read_excel(quickbooks_file)
            df_bridge = pd.read_excel(bridge_file)
            df_customers = pd.read_excel(customers_file)

            df_cb['Invoice Number'] = df_cb['Invoice Number'].astype(str).str.strip().str.lower()
            df_cb['Customer Id'] = df_cb['Customer Id'].astype(str).str.strip().str.lower()
            df_cb['Currency'] = df_cb['Currency'].astype(str).str.strip()
            df_cb['Unit Amount'] = pd.to_numeric(df_cb['Unit Amount'], errors='coerce')
            df_cb['Discount'] = pd.to_numeric(df_cb['Discount'], errors='coerce')
            ...
            # Base: CHARGEBEE
            df_final = df_cb.copy()
            df_final['Invoice No.'] = df_final['Invoice Number']
            ...
            bridge_lookup = dict(zip(bridge_long['lookup_value'], bridge_long['New Account No. for BC']))
            df_final['customer_temp'] = df_final['Customer Id'].map(bridge_lookup)
            df_final['Parent/Customer No.'] = df_final['customer_temp'].fillna('CHECK')
            df_final.drop(columns=['customer_temp'], inplace=True)

            df_final["Subaccount No."] = ""

            # Datas
            df_cb['Date From'] = pd.to_datetime(df_cb['Date From'], errors='coerce')
            df_cb['Date To'] = pd.to_datetime(df_cb['Date To'], errors='coerce')
            df_final['Document Date'] = df_cb['Date From']
            df_final['Posting Date'] = df_cb['Date From']
            df_final['Due Date'] = df_cb['Date From']
            df_final['VAT Date'] = df_cb['Date From']

            # Currency
            df_final['Currency Code'] = df_final['Currency'].apply(lambda x: '' if x == 'CAD' else x)
            ...
            amount_lookup = df_qb.set_index('Invoice Number')['Account #'].to_dict()
            df_final['Account #'] = df_final['Invoice No.'].map(amount_lookup)
            df_final['No.'] = df_final['Account #'].map(account_to_item).fillna("PACKAGE")
            ...
            def get_unit_price(row):
                if row['Currency Code'] == "":
                    return df_qb.loc[df_qb['Invoice Number'] == row['Invoice No.'], 'Product/service amount line'].sum()
                elif str(row['Description']).strip().lower() == "discount":
                    return df_cb[df_cb['Invoice Number'] == row['Invoice No.']]['Discount'].sum()
                else:
                    return row['Unit Amount']

            df_final['Unit Price Excl. VAT'] = df_final.apply(get_unit_price, axis=1)

            df_final['VAT Prod. Posting Group'] = ''
            df_final['Deferral Start Date'] = df_cb['Date From'].fillna('CHECK')
            df_final['Deferral End Date'] = df_cb['Date To'].fillna('CHECK')
            ...
            df_final.loc[mask_small, ['Deferral Code', 'Deferral Start Date', 'Deferral End Date']] = ""
            ...
            with pd.ExcelWriter(output, engine="openpyxl") as writer:
                ...
                for col_name in ["Document Date", "Posting Date", "Due Date", "VAT Date", "Deferral Start Date", "Deferral End Date"]:
            ...
            st.download_button(
                ...
                data=output,
                file_name=f"Hotello_Invoices_{datetime.now().strftime('%Y-%m-%d')}.xlsx",
...
# ================================
    ...
    if chargebee_file and quickbooks_file and bridge_file:
        ...
        try:
            ...
            if 'Entity Type' in df_cb_cm.columns:
                df_cb_cm = df_cb_cm[df_cb_cm['Entity Type'].str.strip().str.lower() != 'tax']
